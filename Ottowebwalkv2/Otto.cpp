...
void Otto::init(int YL, int YR, int RL, int RR, int LA, int RA, bool load_calibration,int Buzzer) {
...
//-- OSCILLATORS TRIMS ------------------------------------------//
///////////////////////////////////////////////////////////////////
void Otto::setTrims(int YL, int YR, int RL, int RR, int LA, int RA) {
  servo[0].SetTrim(YL);
  servo[1].SetTrim(YR);
  servo[2].SetTrim(RL);
  servo[3].SetTrim(RR);
...
void Otto::oscillateServos(int A[6], int O[6], int T, double phase_diff[6], float cycle=1){
...
void Otto::_execute(int A[6], int O[6], int T, double phase_diff[6], float steps = 1.0){
...
void Otto::home(){
  ...
  if(isOttoResting==false){ //Go to rest position only if necessary
    ...
    int homes[6]={90, 90, 90, 90, 150, 30}; //All the servos at rest position
...
void Otto::jump(float steps, int T){
  ...
  int up[]={90,90,150,30,90,90}; // left and right arm down
  _moveServos(T,up);
  int down[]={90,90,90,90,150,30};
  _moveServos(T,down);
}
...
void Otto::walk(float steps, int T, int dir){
  ...
  int A[6]= {30, 30, 20, 20, 30, 30};
  int O[6] = {0, 0, 4, -4, 0 ,0};
  double phase_diff[6] = {0, 0, DEG2RAD(dir * -90), DEG2RAD(dir * -90), 0,DEG2RAD(dir * -90)};
...
void Otto::turn(float steps, int T, int dir){
  ...
  int O[6] = {0, 0, 4, -4, 0, 0};
  double phase_diff[6] = {0, 0, DEG2RAD(-90), DEG2RAD(-90), 0, DEG2RAD(-90)}; 
...
void Otto::bend (int steps, int T, int dir){
  ...
  int bend1[6]={90, 90, 62, 35, 90, 90}; 
  int bend2[6]={90, 90, 62, 105, 45, 135};
  int homes[6]={90, 90, 90, 90, 150, 30};
  ...
  {
    bend1[2]=180-35;
    bend1[3]=180-60;  //Not 65. Otto is unbalanced 60
    bend2[2]=180-105;
    bend2[3]=180-60;
  }
...
void Otto::shakeLeg (int steps,int T,int dir){
  ...
  int shake_leg2[6]={90, 90, 58, 120, 90, 90};
  int shake_leg3[6]={90, 90, 58, 60, 90, 90};
  int homes[6]={90, 90, 90, 90, 150, 30};
...
void Otto::handsup(){
  ...
  int homes[6]={90, 90, 90, 90, 180, -180}; //
    _moveServos(500,homes);   //Move the servos in half a second
	delay(2000);
}

void Otto::handsupdown(){
  int homes[6]={90, 90, 90, 90, 170, -170}; //
  _moveServos(1000,homes);   //Move the servos in half a second
  int homes2[6]={90, 90, 90, 90, -170, 170}; //
  _moveServos(1000,homes2);   //Move the servos in half a second
  int homes3[6]={90, 90, 90, 90, 170, -170}; //
  _moveServos(1000,homes3);   //Move the servos in half a second
  int homes4[6]={90, 90, 90, 90, -170, 170}; //
  _moveServos(1000,homes4);   //Move the servos in half a second
}
...
//-- 
 if(dir==-1)      
  ...
  {
  for (int i=0;i<4;i++){
	int A[6]= {0, 0, 0, 0, 0, -170}; // right hand wave
	int O[6] = {0, 0, 0, 0, 0, -110};
	double phase_diff[6] = {0, 0, 0, 0, 0, DEG2RAD(0)};
    //-- Let's oscillate the servos!
	_execute(A, O, 800, phase_diff, 1);  
  }
